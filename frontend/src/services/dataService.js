...
export async function parseConsumoHistorico(file, diasPeriodo = 90) {
    ...
    // Buscar columnas (E=codigo, F=cantidad, J=bodega, L=nombre)
    const colBodega = _findColumn(headers, [
    ...
    rows.forEach(row => {
        ...
        if (!groups[codigo]) {
            groups[codigo] = { codigo, nombre, total_consumo: 0 }
        }
    ...
    console.log(`[CONSUMO] Productos: ${result.length}, Total consumo: ${result.reduce((s, r) => s + r.total_consumo, 0)}`)
...
export function processRealPipeline(inventario, canastas, consumo, diasProyeccion = 20) {
    ...
    const df = Object.values(codigoMap).map(r => {
        const stock_disponible = Math.max(0, r.stock_actual - r.cantidad_comprometida)
...
export function exportToExcel(reorderData) {
    ...
    const exportRows = reorderData.map(r => ({
        ...
        'Comprometido': r.cantidad_comprometida,
